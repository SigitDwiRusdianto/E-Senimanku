...
class DesainerController extends Controller
{
    ...
    public function halaman_seniman()
    {
        $title = "Seniman";
        ...
        // Mengambil data profile untuk setiap penjual
        $penjualsWithProfile = Penjual::with(['user.profile'])
    ->get()
    ->map(function ($penjual) {
        return [
            'id' => $penjual->id, // Menambahkan properti 'id'
            'nama_toko' => $penjual->nama_toko,
            'deskripsi_toko' => $penjual->deskripsi_toko,
            'alamat' => $penjual->user->profile->alamat,
            'foto' => $penjual->user->profile->foto,
        ];
    });
...
$rekomendasi = Product::with(['product_galleries'])
    ...
    ->get();
            ...
            $kategoris = Product::all();

        return view('pages-user.senimanUs', compact('title', 'penjualsWithProfile', 'rekomendasi', 'kategoris'));
    }
